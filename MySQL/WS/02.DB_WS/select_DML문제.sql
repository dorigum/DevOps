...
-- 1. emp 테이블에서 각 사원 emp_name의 급여(sal)에 100을 더한 후 12를 곱한 값이 출력되도록  select절에 산술식을 사용해보세요.(별칭- 연봉)
...
-- 5.emp 테이블에서 급여가 3000이상 5000이하인 모든 사원의 이름과 급여를 출력하세요.
select emp_name, sal from emp where sal between 3000 and 5000 ;

-- 6.emp 테이블에서 관리자번호(mgr_id)가 6311, 6361, 6351가운데 하나인 모든 사원의 사원번호, 관리자번호, 이름 ,부서번호를 출력하세요.
...
select * from emp where job not in('사무직', '세일즈');

-- 10. emp 테이블에서 급여가 1500이상 2500이하가  아닌 모든 사원의 정보를 검색하세요.
...
select * from emp where bonus = 0;

-- 16. 직업이 ‘직’ 으로 끝나면서 급여가 2000~3000사이 인 사원의 이름을 ‘장동건’, 급여를 3500으로 변경하세요.
update emp set emp_name = '장동건', sal = 3500 where sal between 2000 and 3000 and job like '%직';
select * from emp where emp_name = '장동건';

-- 17. emp_name에 ‘철’자가 들어가면서 직급이 부장인 사원의 정보를 삭제하세요.
delete from emp where emp_name like '%철%' and job = '부장';
select * from emp where emp_name like '%철%' and job = '부장';
