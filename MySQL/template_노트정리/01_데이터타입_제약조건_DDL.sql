...
-- 테이블 생성
 create table student(
   st_no int primary key , -- not null, 중복x 
...
insert into student values(1,'희정',20, now());
insert into student(st_no, st_name,reg_date) values(2,'희정', '2025-12-20');

-- pk특징 테스트
insert into student values(1,'희정2',22, now()); -- X 중복
insert into student( st_name,reg_date) values('희정3', '2025-12-21'); -- x 학번 null안됨

-- 한번에 여러개 레코드 추가
insert into student values (3,'희정',20, now()),(4,'희정',26, now()),(5,'희정',29, now());
...
-- 레코드 등록
insert into member values('jang','희정','1111',20, now());
insert into member values('JaNg','희정','1111',20, now());
-- mysql은 컬럼에 있는 대소문자를 가리지 않는다!! (oracle은 컬럼에 값에 대소문자 가린다)

insert into member values('JaNg2','희정2','1111',22, now()); -- 성공
insert into member values('JaNg2','희정2','2222',22, now());
...
-- 부모에 존재하지 않는 값을 넣어본다.
INSERT INTO EMP(EMP_NO,ENAME, DEPT_CODE) VALUES(500 , '효리','A09');
